...
def generate_gsa_map(manifest: str, genomic_frame: pd.DataFrame) -> dict[str, pd.DataFrame]:
    map = {}
    ...
    manifest = manifest.loc[list(set(manifest.index).intersection(set(genomic_frame.index)))]
...
def generate_methylation_map(manifest: str, methylation_frame: pd.DataFrame) -> dict[str, pd.DataFrame]:
    map = {}
    
    manifest =  pd.read_csv(manifest, skiprows=7, low_memory=False, index_col=0)
    manifest = manifest.loc[list(set(manifest.index).intersection(set(methylation_frame.index)))]
...
def analyse(chr_):
    ...
    for rsID in tqdm(rss_positions_per_chr.index):
        ...
        cpgs_in_range = cpgs_in_range[cpgs_in_range.abs() <= int(distance)].index.tolist()
        ...
        for cpg, cpg_data in cpgs_in_range.iterrows():
            ...
            stats.append({"CHR": chr_, "rs": rsID, "rs POS": rs_pos, "cpg": cpg, "cpg POS": cpg_pos, "slope": slope, "R2": r**2, "p-value": pval})
